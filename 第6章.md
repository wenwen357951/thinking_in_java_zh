...
這個例子向大家展示了大量特性。首先，在`Cleanser append()`方法裡，字符串同一個`s`連接起來。這是用`+=`運算子實作的。同`+`一樣，`+=`被Java用於對字符串進行“重載”處理。
...
不能指望自己能確切知道何時會開始垃圾收集。垃圾收集器可能永遠不會得到呼叫。即使得到呼叫，它也可能以自己願意的任何順序回收物件。除此以外，Java 1.0實作的垃圾收集器機制通常不會呼叫`finalize()`方法。除內存的回收以外，其他任何東西都最好不要依賴垃圾收集器進行回收。若想明確地清除什麼，請製作自己的清除方法，而且不要依賴`finalize()`。然而正如以前指出的那樣，可強迫Java1.1呼叫所有終結模塊（`Finalizer`）。
