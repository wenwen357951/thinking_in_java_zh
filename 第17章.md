...
另一種特殊情況與`package`關鍵字有關。儘管Java是一種自由形式的語言，但這個程式要求`package`關鍵字必須位於行首。若發現`package`關鍵字，就通過檢查位於開頭的空格以及位於末尾的分號，從而提取出包名（注意亦可一次單獨的操作實作，方法是使用重載的`substring()`，令其同時檢查起始和結束索引位置）。隨後，將包名中的點號替換成特定的文件分隔符——當然，這裡要假設文件分隔符僅有一個字符的長度。儘管這個假設可能對目前的所有系統都是適用的，但一旦遇到問題，一定不要忘了檢查一下這裡。
