...
(40) 通過事先的約定，C++中經常出現的`const`問題在Java裡已得到了控制。我們只能傳遞指向物件的引用，本地副本永遠不會為我們自動生成。若希望使用類似C++按值傳遞那樣的技術，可呼叫`s`，生成參數的一個本地副本（儘管`clone()`的設計依然尚顯粗糙——參見第12章）。根本不存在被自動呼叫的副本構造器。為創建一個編譯期的常數值，可象下面這樣編碼：
