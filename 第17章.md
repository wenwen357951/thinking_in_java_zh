...
為了讓這個程序正確運行，首先必須建構一個類名，將它作為一個“倉庫”，負責容納標準Java庫中的所有類名。為達到這個目的，需遍歷用於標準Java庫的所有源碼子目錄，並在每個子目錄都運行`ClassScanner`。至於參數，則提供倉庫文件的名字（每次都用相同的路徑和名字）和命令行開關`-a`，指出類名應當添加到該倉庫文件中。
