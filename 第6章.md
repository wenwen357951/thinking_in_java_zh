...
此時，必要的類已全部裝載完畢，所以能夠創建物件。首先，這個物件中的所有基本資料型態都會設成它們的默認值，而將物件引用設為`null`。隨後會呼叫基類建構子。在這種情況下，呼叫是自動進行的。但也完全可以用`super`來自行指定建構子呼叫（就象在`Beetle()`建構子中的第一個操作一樣）。基類的構建採用與派生類建構子完全相同的處理過程。基礎順建構子完成以後，實例變量會按本來的順序得以初始化。最後，執行建構子剩餘的主體部分。
