...
作為初始化的一種具體操作形式，建構子應使大家明確感受到在語言中進行初始化的重要性。與C++的程式設計一樣，判斷一個程式效率如何，關鍵是看是否由於變量的初始化不正確而造成了嚴重的編程錯誤（Bug）。這些形式的錯誤很難發現，而且類似的問題也適用於不正確的清除或終結工作。由於建構子使我們能保證正確的初始化和清除（若沒有正確的建構子呼叫，編譯器不允許物件創建），所以能獲得完全的控制權和安全性。
