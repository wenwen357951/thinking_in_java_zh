...
新方法是`what()`和`adjust()`。前者返回一個`String`引用，同時返回對那個類別的說明；後者使我們能對每種樂器進行調整。
...
在`tune`中，`InstrumentX i`會發出`play()`訊息，同時將某個`NoteX`成員作為參數使用（`MIDDLE_C`）。由於`NoteX`包含了`int`定義，重載的`play()`方法的`int`版本會得到呼叫。同時由於它尚未被“覆蓋”，所以會使用基類別版本。
...
在類別`Adventure`中，我們可看到共有四個方法，它們將不同的界面和具體類別作為自己的參數使用。創建一個`Hero`物件後，它可以傳遞給這些方法中的任何一個。這意味著它們會依次向上轉換到每一個界面。由於界面是用Java設計的，所以這樣做不會有任何問題，而且程式員不必對此加以任何特別的關注。
...
`Vampire`的語法僅在繼承界面時才可使用。通常，我們只能對單獨一個類別應用`extends`（擴展）關鍵字。但由於界面可能由多個其他界面構成，所以在建構一個新界面時，`extends`可能引用多個基礎界面。正如大家看到的那樣，界面的名字只是簡單地使用逗號分隔。
...
在匿名內部類別中，`Contents`是用一個默認建構子創建的。下面這段程式碼展示了基類別需要含有參數的一個建構子時做的事情：
...
只要自己只是想分配一個字段，上述方法就肯定可行。但假如需要採取一些類別似於建構子的行動，又應怎樣操作呢？通過Java 1.1的實例初始化，我們可以有效地為一個匿名內部類別創建一個建構子：
...
這樣便生成一個獨立的、名為`TestBed$Tester`的類別（為運行程式，請使用`java TestBed$Tester`命令）。可將這個類別用於測試，但不需在自己的最終發行版本中包含它。
...
這個例子簡單地揭示出當我們從外部類別繼承的時候，沒有任何額外的內部類別繼續下去。然而，仍然有可能“明確”地從內部類別繼承：
...
同往常一樣，建構子與其他種類別的方法是有區別的。在涉及到多型性的問題後，這種方法依然成立。儘管建構子並不具有多型性（即便可以使用一種“虛擬建構子”——將在第11章介紹），但仍然非常有必要理解建構子如何在複雜的分級結構中以及隨同多型性使用。這一理解將有助於大家避免陷入一些令人不快的糾紛。
...
這個例子在其他類別的外部創建了一個複雜的類別，而且每個類別都有一個建構子對自己進行了宣佈。其中最重要的類別是`Sandwich`，它反映出了三個級別的繼承（若將從`Object`的默認繼承算在內，就是四級）以及三個成員物件。在`main()`裡創建了一個`Sandwich`物件後，輸出結果如下：
...
`DoBasefinalization`類別只是簡單地容納了一個標誌，向分級結構中的每個類別指出是否應呼叫`super.finalize()`。這個標誌的設置建立在命令行參數的基礎上，所以能夠在進行和不進行基類別終結工作的前提下查看行為。
...
在`Frog.main()`中，`DoBaseFinalization`標誌會得到配置，而且會創建單獨一個`Frog`物件。請記住垃圾收集（特別是終結工作）可能不會針對任何特定的物件發生，所以為了強制採取這一行動，`System.runFinalizersOnExit(true)`添加了額外的開銷，以保證終結工作的正常進行。若沒有基類別初始化，則輸出結果是：
...
從中可以看出確實沒有為基類別·呼叫終結模塊。但假如在命令行加入`finalize`參數，則會獲得下述結果：
...
儘管成員物件按照與它們創建時相同的順序進行終結，但從技術角度說，並沒有指定物件終結的順序。但對於基類別，我們可對終結的順序進行控制。採用的最佳順序正是在這裡採用的順序，它與初始化順序正好相反。按照與C++中用於“析構器”相同的形式，我們應該首先執行派生類別的終結，再是基類別的終結。這是由於派生類別的終結可能呼叫基類別中相同的方法，要求基類別組件仍然處於活動狀態。因此，必須提前將它們清除（析構）。

## 7.7.3 建構子內部的多型性方法的行為

建構子呼叫的分級結構（順序）為我們帶來了一個有趣的問題，或者說讓我們進入了一種進退兩難的局面。若當前位於一個建構子的內部，同時呼叫準備建構的那個物件的一個動態綁定方法，那麼會出現什麼情況呢？在原始的方法內部，我們完全可以想象會發生什麼——動態綁定的呼叫會在運行期間進行解析，因為物件不知道它到底從屬於方法所在的那個類別，還是從屬於從它派生出來的某些類別。為保持一致性，大家也許會認為這應該在建構子內部發生。

但實際情況並非完全如此。若呼叫建構子內部一個動態綁定的方法，會使用那個方法被覆蓋的定義。然而，產生的效果可能並不如我們所願，而且可能造成一些難於發現的程式錯誤。

從概念上講，建構子的職責是讓物件實際進入存在狀態。在任何建構子內部，整個物件可能只是得到部分組織——我們只知道基類別物件已得到初始化，但卻不知道哪些類別已經繼承。然而，一個動態綁定的方法呼叫卻會在分級結構裡“向前”或者“向外”前進。它呼叫位於派生類別裡的一個方法。如果在建構子內部做這件事情，那麼對於呼叫的方法，它要操縱的成員可能尚未得到正確的初始化——這顯然不是我們所希望的。
...
(2) 就象前面敘述的那樣，呼叫基類別建構子。此時，被覆蓋的`draw()`方法會得到呼叫（的確是在`RoundGlyph`建構子呼叫之前），此時會發現`radius`的值為0，這是由於步驟(1)造成的。

(3) 按照原先聲明的順序呼叫成員初始化程式碼。

(4) 呼叫派生類別建構子的主體。

採取這些操作要求有一個前提，那就是所有東西都至少要初始化成零（或者某些特殊資料型態與“零”等價的值），而不是僅僅留作垃圾。其中包括通過“組合”技術嵌入一個類別內部的物件引用。如果假若忘記初始化那個引用，就會在運行期間出現異常事件。其他所有東西都會變成零，這在觀看結果時通常是一個嚴重的警告信號。
...
因此，設計建構子時一個特別有效的規則是：用盡可能簡單的方法使物件進入就緒狀態；如果可能，避免呼叫任何方法。在建構子內唯一能夠安全呼叫的是在基類別中具有`final`屬性的那些方法（也適用於`private`方法，它們自動具有`final`屬性）。這些方法不能被覆蓋，所以不會出現上述潛在的問題。


# 7.8 通過繼承進行設計


學習了多型性的知識後，由於多型性是如此“聰明”的一種工具，所以看起來似乎所有東西都應該繼承。但假如過度使用繼承技術，也會使自己的設計變得不必要地複雜起來。事實上，當我們以一個現成類別為基礎建立一個新類別時，如首先選擇繼承，會使情況變得異常複雜。

一個更好的思路是首先選擇“組合”——如果不能十分確定自己應使用哪一個。組合不會強迫我們的程式設計進入繼承的分級結構中。同時，組合顯得更加靈活，因為可以動態選擇一種類別型（以及行為），而繼承要求在編譯期間準確地知道一種類別型。下面這個例子對此進行了闡釋：
...
一條常規的設計準則是：用繼承表達行為間的差異，並用成員變量表達狀態的變化。在上述例子中，兩者都得到了應用：繼承了兩個不同的類別，用於表達`act()`方法的差異；而`Stage`通過組合技術允許它自己的狀態發生變化。在這種情況下，那種狀態的改變同時也產生了行為的變化。

## 7.8.1 純繼承與擴展

學習繼承時，為了創建繼承分級結構，看來最明顯的方法是採取一種“純粹”的手段。也就是說，只有在基類別或“界面”中已建立的方法才可在派生類別中被覆蓋，如下面這張圖所示：

![](/image/7-6.gif)

可將其描述成一種純粹的“屬於”關係，因為一個類別的界面已規定了它到底“是什麼”或者“屬於什麼”。通過繼承，可保證所有派生類別都只擁有基類別的界面。如果按上述示意圖操作，派生出來的類別除了基類別的界面之外，也不會再擁有其他什麼。

可將其想象成一種“純替換”，因為派生類別物件可為基類別完美地替換掉。使用它們的時候，我們根本沒必要知道與子類別有關的任何額外信息。如下所示：

![](/image/7-7.gif)

也就是說，基類別可接收我們發給派生類別的任何訊息，因為兩者擁有完全一致的界面。我們要做的全部事情就是從派生向上轉換，而且永遠不需要回過頭來檢查物件的準確類別型是什麼。所有細節都已通過多型性獲得了完美的控制。

若按這種思路考慮問題，那麼一個純粹的“屬於”關係似乎是唯一明智的設計方法，其他任何設計方法都會導致混亂不清的思路，而且在定義上存在很大的困難。但這種想法又屬於另一個極端。經過細緻的研究，我們發現擴展界面對於一些特定問題來說是特別有效的方法。可將其稱為“類別似於”關係，因為擴展後的派生類別“類別似於”基類別——它們有相同的基礎界面——但它增加了一些特性，要求用額外的方法加以實現。如下所示：

![](/image/7-8.gif)

儘管這是一種有用和明智的做法（由具體的環境決定），但它也有一個缺點：派生類別中對界面擴展的那一部分不可在基類別中使用。所以一旦向上轉換，就不可再呼叫新方法：

![](/image/7-9.gif)

若在此時不進行向上轉換，則不會出現此類別問題。但在許多情況下，都需要重新核實物件的準確類別型，使自己能訪問那個類別型的擴展方法。在後面的小節裡，我們具體講述了這是如何實現的。

## 7.8.2 向下轉換與運行期類別型識別

由於我們在向上轉換（在繼承結構中向上移動）期間丟失了具體的類別型信息，所以為了獲取具體的類別型信息——亦即在分級結構中向下移動——我們必須使用 “向下轉換”技術。然而，我們知道一個向上轉換肯定是安全的；基類別不可能再擁有一個比派生類別更大的界面。因此，我們通過基類別界面發送的每一則訊息都肯定能夠接收到。但在進行向下轉換的時候，我們（舉個例子來說）並不真的知道一個幾何形狀實際是一個圓，它完全可能是一個三角形、方形或者其他形狀。

![](/image/7-10.gif)

為解決這個問題，必須有一種辦法能夠保證向下轉換正確進行。只有這樣，我們才不會冒然轉換成一種錯誤的類別型，然後發出一條物件不可能收到的訊息。這樣做是非常不安全的。

在某些語言中（如C++），為了進行保證“類別型安全”的向下轉換，必須採取特殊的操作。但在Java中，所有轉換都會自動得到檢查和核實！所以即使我們只是進行一次普通的括弧轉換，進入運行期以後，仍然會毫無留情地對這個轉換進行檢查，保證它的確是我們希望的那種類別型。如果不是，就會得到一個`ClassCastException`（類別轉換異常）。在運行期間對類別型進行檢查的行為叫作“運行期類別型識別”（RTTI）。下面這個例子向大家演示了RTTI的行為：
...
和在示意圖中一樣，`MoreUseful`（更有用的）對`Useful`（有用的）的界面進行了擴展。但由於它是繼承來的，所以也能向上轉換到一個`Useful`。我們可看到這會在對數組`x`（位於`main()`中）進行初始化的時候發生。由於數組中的兩個物件都屬於`Useful`類別，所以可將`f()`和`g()`方法同時發給它們兩個。而且假如試圖呼叫`u()`（它只存在於`MoreUseful`），就會收到一條編譯期出錯提示。

若想訪問一個`MoreUseful`物件的擴展界面，可試著進行向下轉換。如果它是正確的類別型，這一行動就會成功。否則，就會得到一個`ClassCastException`。我們不必為這個異常編寫任何特殊的程式碼，因為它指出的是一個可能在程式中任何地方發生的一個編程錯誤。

RTTI的意義遠不僅僅反映在轉換處理上。例如，在試圖向下轉換之前，可通過一種方法瞭解自己處理的是什麼類別型。整個第11章都在講述Java運行期類別型識別的方方面面。


# 7.9 總結


“多型性”意味著“不同的形式”。在物件導向的程式設計中，我們有相同的外觀（基類別的通用界面）以及使用那個外觀的不同形式：動態綁定或組織的、不同版本的方法。

通過這一章的學習，大家已知道假如不利用數據抽象以及繼承技術，就不可能理解、甚至去創建多型性的一個例子。多型性是一種不可獨立應用的特性（就象一個`switch`語句），只可與其他元素協同使用。我們應將其作為類別總體關係的一部分來看待。人們經常混淆Java其他的、非物件導向的特性，比如方法重載等，這些特性有時也具有物件導向的某些特徵。但不要被愚弄：如果以後沒有綁定，就不成其為多型性。

為使用多型性乃至物件導向的技術，特別是在自己的程式中，必須將自己的編程視野擴展到不僅包括單獨一個類別的成員和訊息，也要包括類別與類別之間的一致性以及它們的關係。儘管這要求學習時付出更多的精力，但卻是非常值得的，因為只有這樣才可真正有效地加快自己的編程速度、更好地組織程式碼、更容易做出包容面廣的程式以及更易對自己的程式碼進行維護與擴展。


# 7.10 練習

(1) 創建`Rodent`（齧齒動物）:`Mouse`（老鼠）,`Gerbil`（鼴鼠）,`Hamster`（大頰鼠）等的一個繼承分級結構。在基類別中，提供適用於所有`Rodent`的方法，並在派生類別中覆蓋它們，從而根據不同類別型的`Rodent`採取不同的行動。創建一個`Rodent`數組，在其中填充不同類別型的`Rodent`，然後呼叫自己的基類別方法，看看會有什麼情況發生。

(2) 修改練習1，使`Rodent`成為一個界面。

(3) 改正`WindError.java`中的問題。

(4) 在`GreenhouseControls.java`中，添加`Event`內部類別，使其能打開和關閉風扇。
