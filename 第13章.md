...
在Java 1.0中，圖形用戶界面（GUI）庫最初的設計目標是讓程序員構建一個通用的GUI，使其在所有平臺上都能正常顯示。
...
最重要的是我們會注意到`BangBean`執行了這種串聯化的界面。這意味著應用程序構建工具可以在程序設計者調整完屬性值後利用串聯為`BangBean`貯藏所有的信息。當Bean作為運行的應用程序的一部分被創建時，那些被貯藏的屬性被重新恢復，因此我們可以正確地得到我們的設計。
