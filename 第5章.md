...
我們通常認為訪問控制是“隱藏實作細節”的一種方式。將數據和方法封裝到類內後，可生成一種數據類型，它具有自己的特徵與行為。但由於兩方面重要的原因，訪問為那個數據類型加上了自己的邊界。第一個原因是規定客戶程序員哪些能夠使用，哪些不能。我們可在結構裡構建自己的內部機制，不用擔心客戶程序員將其當作界面的一部分，從而自由地使用或者“濫用”。
...
有兩方面的原因要求我們控制對成員的訪問。第一個是防止用戶接觸那些他們不應碰的工具。對於數據類型的內部機制，那些工具是必需的。但它們並不屬於用戶界面的一部分，用戶不必用它來解決自己的特定問題。所以將方法和字段變成“私有”（`private`）後，可極大方便用戶。因為他們能輕易看出哪些對於自己來說是最重要的，以及哪些是自己需要忽略的。這樣便簡化了用戶對一個類的理解。
